...
# Load models with simulated priors
...
# 
# name                            pr > 1
# Simple oxygen only	            5.2			
# Noninvasive ventilation	        11.4			
# Invasive mechanical ventilation	27.2	
...
samples_size = 
  simulated_priors_models[[1]] %>% 
  tidy_draws() %>% 
  nrow()

priors = data.frame()
posteriors = tibble(x = rep(NA, samples_size))
...
prior =
  ...
  select(b_Intercept) %>% 
  median_hdi() %>% 
  summarise(lower = exp(.lower),
            upper = exp(.upper),
            model = labels[i])
  
  priors = priors %>% bind_rows(prior)
...
p1 = 
  ...
  ggplot(aes(y = Model, x = exp(Posterior),
  ...
  scale_x_continuous(breaks = seq(0.6, 1.3, 0.1)) +
  coord_cartesian(x = c(0.6, 1.3)) +
  theme(
    strip.background = element_rect(fill = "#E4E6E7"),
    strip.text.x = element_text(size = 12),
    axis.ticks.x = element_blank(),
