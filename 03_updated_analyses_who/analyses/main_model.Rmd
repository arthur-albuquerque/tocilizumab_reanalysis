...
ventilation ($\alpha$).
Both coefficients are multiplied by $x_i$, which is dummy-coded. Lastly,
...
$\beta_{NIV}$, because we did not expect large differences between the invasive
mechanical subgroup ($\alpha$) and the other two respiratory support subgroups.
...
($\alpha$, $\beta_{SOO}, \beta_{NIV}$ and $\tau$), we will use three different
types of prior:
...
Here are the distributions used for $\beta_{SOO}$ and $\beta_{NIV}$ 
(hereafter, **"Coefficients"**):
...
```{r  fig.align='center', fig.cap="Panel A: Prior distributions; Panel B: Posterior distributions"}
priors + posteriors + 
  plot_annotation(tag_levels = "A")
```

"Percent relative deviation computation can be computed using the following
formula: [(model with initial subjective parameter) – (model with
noninformative prior)/(model with initial subjective parameter)] X 100."
