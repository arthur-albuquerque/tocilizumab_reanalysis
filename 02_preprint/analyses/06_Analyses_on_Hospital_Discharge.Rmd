...
**In this document, we will show most of the analyses regarding hospital discharge, except sensitivity analyses using different baseline risks. Thus, we will show Supplementary Figures 5-6 and Supplementary Tables 7-8.**

First, we will derive the risk difference from odds ratio. Next,
...
Here are the 95% highest density intervals in *odds ratio* for each distribution from
the figure above.
...
# tfinal %>% 
#  gtsave(here("02_preprint", "output", "plots", "appendix", # File path
#                  "STable6_discharge_hdi_intervals_figureS3.pdf")) # File name
...
```{r , fig.align='center', fig.width=10, fig.height = 8.5, warning=FALSE, fig.cap="Panels A and C: Point estimates depict the median. Interval bars depict the 95% highest density intervals. Panels B and D: Cumulative posterior distributions correspond to the probabilities that the risk difference (RD) is less than or equal to the effect size on the X‐axis."}
p1_overlay = p1 + inset_element(arrows_plot,
                                ignore_tag = TRUE,
                                align_to = "full",
                                left = unit(0.65, 'cm'),
...
t = tibble(
  ...
  "Pr(< -3%)" = c(round((samples_posteriors_not_using_discharge %>%
                           filter(`Underlying_Prior` == "Non-informative") %>%
                           summarise(n = sum(RD < -0.03) / n()) %>%
                           pull()), 2) * 100,
...
tfinal = tfinal %>% 
  ...
  tab_spanner(label = "Propability of Benefit",
              columns = c("Pr(< -3%)", "Pr(< -2%)", "Pr(< -1%)", "Pr(< 0%)")) %>% 
  tab_spanner(label = "Probability of Harm",
              columns = c("Pr(> 1%)", "Pr(> 2%)", "Pr(> 3%)")) %>%
  cols_align(
    align = "center",
    columns = everything()
  ) 
...
t = tibble(
  ...
  "Pr(< -3%)" = c(round((samples_posteriors_using_discharge %>%
                           filter(`Underlying_Prior` == "Non-informative") %>%
                           summarise(n = sum(RD < -0.03) / n()) %>%
                           pull()), 2) * 100,
...
t = tibble(
  ...
  "Pr(< -3%)" = c(round((samples_posteriors_simple_discharge %>%
                           filter(`Underlying_Prior` == "Non-informative") %>%
                           summarise(n = sum(RD < -0.03) / n()) %>%
                           pull()), 2) * 100,
...
t = tibble(
  ...
  "Pr(< -3%)" = c(round((samples_posteriors_noninvasive_discharge %>%
                           filter(`Underlying_Prior` == "Non-informative") %>%
                           summarise(n = sum(RD < -0.03) / n()) %>%
                           pull()), 2) * 100,
...
t = tibble(
  ...
  "Pr(< -3%)" = c(round((samples_posteriors_invasive_discharge %>%
                           filter(`Underlying_Prior` == "Non-informative") %>%
                           summarise(n = sum(RD < -0.03) / n()) %>%
                           pull()), 2) * 100,
...
```{r}
not_using = samples_posteriors_not_using_discharge %>% 
  mutate(subgroup = "Not using corticosteroids") %>% 
  select(subgroup, `Underlying_Prior`, RD)

using = samples_posteriors_using_discharge %>% 
  mutate(subgroup = "Using corticosteroids") %>% 
  select(subgroup, `Underlying_Prior`, RD)

simple = samples_posteriors_simple_discharge %>% 
  mutate(subgroup = "Simple oxygen only") %>% 
  select(subgroup, `Underlying_Prior`, RD)

noninvasive = samples_posteriors_noninvasive_discharge %>% 
  mutate(subgroup = "Non-invasive ventilation") %>% 
  select(subgroup, `Underlying_Prior`, RD)

invasive = samples_posteriors_invasive_discharge %>% 
...
# t %>% 
#   gtsave(here("02_preprint", "output", "plots", "appendix", # File path
#                  "STable7_discharge_hdi_figure3.pdf"))
```
...
# t %>% 
#   gtsave(here("02_preprint", "output", "plots", "appendix", # File path
#                  "STable8_discharge_probabilities_figure3.pdf"))
