...
**In this document, we will discuss about the underlying analyses of Supplementary Figures 2 and 4 about sensitivity analyses using different baseline risks. We will only focus on the mortality outcome**
...
$r_{0}$ is the outcome risk in  control arm in RECOVERY and $O R$ is odds ratio.

$$R D=\frac{r_{0}\left(r_{0}-1\right)\left(O R-1\right)}{r_{0} O R-r_{0}+1}$$

In the document ["02_Calculating_Posterior_Distribution"](https://github.com/arthur-albuquerque/tocilizumab_reanalysis/tree/master/02_preprint/analyses), we used the risk in RECOVERY's
control group as the baseline risk (BR) for each respective subgroup.
...
p1 = plot_baseline_risks(multiple_baseline_risks_not_using_death) +
  scale_y_discrete(breaks = c(15, 35, 55))
```
...
p3 = multiple_baseline_risks_simple_death %>% 
  ...
  theme(plot.title.position = 'plot',
    ...
    scale_x_continuous(breaks = seq(-12, 12, 4))+
    coord_cartesian(x = c(-12,12)) +
    scale_y_discrete(breaks = c(3, 23, 43))
```
...
$3$%. The **idea** for the following analysis was heavily influenced by 
...
knitr::include_graphics(rep(here("02_preprint", "analyses", "doi_et_al_figures.png")))
```
