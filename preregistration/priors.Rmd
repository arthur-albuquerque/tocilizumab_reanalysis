...
Subgroup analyses are fundamental for clinical research. They allow one to
...
discrepancy to ascertain different treatments for specific subgroups. Notably,
frequentist subgroup analyses pose several limitations. Their low statistical
power increases both type 2 error, leading to false-negatives results
[@schulz2005], and type 1 error risk, inflating treatment effect
[@ioannidis2005]. It is estimated that clinical trials designed to have 80%
power for the primary outcome is anticipated to have 30% power for the
...
interval [@rafi2020]. A single CI does not provide any information on the
actual effect size. CIs can only inform about the compatibility of data from
this trial to an arbitrary threshold (e.g., 95%). It does not indicate what the
probability of a range of effects is (e.g., RR \< 1, 0.9, 0.8 etc.) to be true.
Thus, the frequentist statistical framework poorly aids clinicians to analyze
data from subgroups in RCTs thoroughly.
...
because the RECOVERY group estimated risk difference was $-0.05$ for their
...
To aid in the interpretation of our methods, we will simulate data to create
multiple priors. In this example, we will analyze the mortality outcome of a
...
mentioned above. The priors can be visualized in *Figure 1* and their parameters
in *Table 1*.
