...
> We will perform a **reanalysis** of the RECOVERY trial on tocilizumab.

## Rationale underlying our priors

We have decided to **define** the parameters of our **priors** for each **subgroup** on two fronts:

1. We will perform a **systematic review** of randomized controlled trials (RCTs) on tocilizumab and COVID-19
   * PROSPERO registration number .....
   * To create a **data derived prior**
      + We will create **subgroup-specific** data derived **priors** based on the **characteristics** of the **RCTs** from the systematic review
        - For example, suppose an RCT only has data on mechanical ventilation. In that case, we will only use this data for the data derived prior on the "invasive mechanical ventilation" subgroup from the tocilizumab trial. 
        - Another example: suppose an RCT has data on two subgroups. If possible, we will separate the data and include or delete it according to the subgroups' characteristics from the tocilizumab trial.
   * We **will not** further **describe** this prior here since the systematic review has not been performed yet
...
tibble(
  ...
  "Moderate" = c("Pr(ARD > 0.1) & Pr(ARD < -0.1) = 0.05", "Pr(ARD > 0) = 0.15", "Pr(ARD < 0) = 0.15"),
    
  "Strong" = c("Pr(ARD > 0.075) & Pr(ARD < -0.075) = 0.05", "Pr(ARD > 0) = 0.05", "Pr(ARD < 0) = 0.05")
)  %>% 
...
We will also **reanalyze** the tocilizumab trial's **overall results**. To this end, we will **compile** all of the **results** from the RCTs in our **systematic review** to create a **data derived prior**. We will also use the **dexamethasone trial** as a reference for the **other priors**.
...
## Priors: all patients {.panel-name}
