...
t1 = data %>% 
  ...
  mutate(Baseline_risk = as.character(Baseline_risk))
  
  # Plot!
  t1 %>% 
  ggplot(aes(x = 100*Risk_difference, y = Baseline_risk))+
...
p5 = multiple_baseline_risks_invasive_discharge %>% 
  ...
  filter(!str_detect(Baseline_risk, "-"),
         Baseline_risk != "RD_0") %>% 
   # Isolate the Baseline risk from the string (it takes a long time)
   separate(Baseline_risk,
     sep = "_",
  ...
  mutate(Baseline_risk = as.character(Baseline_risk)) %>% 
  # Set order
  mutate(Baseline_risk = factor(Baseline_risk, levels = seq(1, 36, 1))) %>% 
...
```{r include=FALSE}
# Plot to remove title and save it
# p1 + p2 
# 
# ggsave(width = 8,
#        height = 5,
...
#        device=cairo_pdf, 
#        here("final_analyses", "output", "plots", "appendix", # File path
#                   "SFigure23_discharge_varying_baseline_probabilities_corticosteroids.pd# f")) # File # name
