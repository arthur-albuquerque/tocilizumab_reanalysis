---
title: "04_Sensitivity_Analyses_Different_Baseline_Risks"
...
```{r}
p1 = plot_baseline_risks(multiple_baseline_risks_not_using_death,
                         "Not using corticosteroids")
```

```{r}
p2 = plot_baseline_risks(multiple_baseline_risks_using_death,
                         "Using corticosteroids")
```

```{r}
# Full code in this code chunk because I had to add an extra line to set the order
p3 = multiple_baseline_risks_simple_death %>% 
  as.data.frame() %>% 
  filter.(`Underlying_Prior` == "Evidence-based") %>% 
  select.(starts_with("RD")) %>% 
  pivot_longer.(
  ...
  mutate.(BR = as.character(BR)) %>% 
  # Set order
  mutate.(BR = factor(BR, levels = seq(3, 43, 1))) %>% 
  filter.(BR != "NA") %>%
...
```{r}
p4 = plot_baseline_risks(multiple_baseline_risks_noninvasive_death,
                         "Non-invasive ventilation")
```

```{r}
p5 = plot_baseline_risks(multiple_baseline_risks_invasive_death,
                         "Invasive mechanical ventilation")
```
...
```{r, fig.align='center', fig.height=5, fig.width=8, fig.cap="Sensitivity analyses using multiple different baseline risks for subgroups on use of corticosteroids. Each line represents the posterior probability of benefit  for a specific cutoff, such as risk difference greater or equal to 0%, 1%, 2% and 5%. Vertical dashed lines represent the respective baseline risk (Table 1) used in other analyses. Panel A shows results for patients not using corticosteroids; Panel B shows results for patients using corticosteroids."}
...
```{r, fig.align='center', fig.height=5, fig.width=8, fig.cap="Sensitivity analyses using multiple different baseline risks for subgroups on respiratory support. Each line represents the posterior probability of benefit  for a specific cutoff, such as risk difference greater or equal to 0%, 1%, 2% and 5%. Vertical dashed lines represent the respective baseline risk (Table 1) used in other analyses. Panel A shows results for simple oxygen only; Panel B shows results for non-invasive ventilation; Panel C shows results for invasive mechanical ventilation."}
