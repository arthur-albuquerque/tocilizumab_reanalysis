...
```{r}
## Same table as above, but in this case I made it "manually"
...
probabilities = function(name # Subgroup (within quotes)
  ...
  summarise("Pr(< -5%)" = sum(RD < -0.05) / n(),
            ...
            "Pr(> 2%)" =  sum(RD > 0.02) / n(),
            "Pr(> 5%)" =  sum(RD > 0.05) / n()) %>% 
  mutate(across(`Pr(< -5%)`:`Pr(> 5%)`, ~multiply100(.))) %>%
  mutate(across(`Pr(< -5%)`:`Pr(> 5%)`, ~round(., 1))) %>%
...
# t %>% 
#   gtsave(here("final_analyses", "output", "plots", "appendix", # File path
#                  "STable4_probabilities_figure3.png"))
